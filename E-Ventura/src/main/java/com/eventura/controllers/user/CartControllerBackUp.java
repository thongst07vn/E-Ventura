...
import com.eventura.dtos.CartItemsBackUpDTO;
...
public class CartControllerBackUp {
	...
	public String cartList(Authentication authentication, ModelMap modelMap, @AuthenticationPrincipal UserDetails userDetails) {
		...
		Map<Vendors, List<CartItemsBackUpDTO>> cartItemsGroupedByVendor = new HashMap<>();
		Map<String, CartItemsBackUpDTO> cartItemDtoLookupMap = new HashMap<>();
		...
		for(Carts cart : userCarts) {
			...
			for(CartItems cartItem: sortedCartItems) { // Lặp qua danh sách đã sắp xếp
				...
				CartItemsBackUpDTO dto = new CartItemsBackUpDTO(cartItem, afterDiscountPrice, originalPrice, hasDiscount);
